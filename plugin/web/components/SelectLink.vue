<template>
    <el-row>
            ...
            <el-tabs v-model="tabname" type="card" @tab-click="changeTabs">
                <el-tab-pane label="页面" name="1">
                    <el-table :data="pageData" stripe style="margin-bottom:60px">
                        <el-table-column prop="name" label="页面" align="center">
                        </el-table-column>
        ...
        <el-dialog title="集赞活动" :visible.sync="jizanModal" width='800px' top="20px" :modal-append-to-body="false">
            ...
            <el-row>
                <el-pagination background layout="total,prev,pager,next" :current-page.sync="pagination.page" :total="pagination.rowCount" :page-size="pagination.pageSize" @current-change="changePage" style="float:right;margin:24px 0"></el-pagination>
            </el-row>
            <p slot="footer"></p>
        </el-dialog>
        <!-- 小程序 -->
        ...
        <!-- 文章 -->
        <el-dialog title="选择文章" :visible.sync="showArticle" width='800px' top="20px" :modal-append-to-body="false">
            <el-alert title="提示" :closable="false" show-icon v-show='!articleData.length' style="margin-top:-24px">
                <p>暂无文章，请先去
            ...
            <el-table :data="articleData" stripe style="margin-top:24px">
                <el-table-column prop="content" label="内容" header-align="center" width="400">
                    <template slot-scope="scope">
                        <div style="display:flex">
                            <img :src="scope.row.img[0].img_url" alt="" style="width:120px;height:100px" v-if="scope.row.img.length">
                ...
                <el-table-column prop="pay_num" label="购买数" align="center">
                </el-table-column>
                ...
                <el-table-column label="操作" align="center">
                    <template slot-scope="scope">
                        <el-button type="text" size="mini" @click="selectArticle(scope.row)" v-if="scope.row.status">选择</el-button>
...
export default {
    ...
    methods: {
        ...
        okUmp() {
            ...
            if (this.umpSelect == 3) {
                //文章
                this.$confirm("请选择跳转到文章列表页或文章详情页", "提示", {
