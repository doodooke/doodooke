<template>
    <el-row @mouseenter.native='showBorder=true' @mouseleave.native='showBorder=false' class="components">
        ...
        <el-row style="padding:20px 0;position:relative" :style="{paddingLeft:formProps.padding+'px',paddingRight:formProps.padding+'px',height:formProps.height+'px'}">
            <draggable v-model='componentData' :options="{group:'diy'}" @change='changeComponent' class="layout-components">
                <transition-group tag="ul" style='width:auto;min-height:100px;padding:0;'>
                    <component v-for='(item,index) in componentData' :key='index' :is='item.component' :item-data="item.data" @click.native.stop='clickComponent(item)' @save-comp='addData' @del-comp="delSubComponent" style="position:absolute">
                    </component>
                </transition-group>
            </draggable>
        </el-row>
        <el-row v-if="itemData.id==$store.state.componentId" class="config" :style="{left: configOffsetLeft + 'px'}">
            <el-row class="config-title">自由面板</el-row>
            <el-form label-width="80px">
                <el-form-item label="高度">
                    <el-slider v-model="formProps.height" :min="100" :max="1000" show-input></el-slider>
                </el-form-item>
